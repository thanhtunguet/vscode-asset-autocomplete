...
export function registerJsonSortCommands(
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      'i18n-autocomplete.sortJsonTranslations',
      async () => {
        try {
          vscode.window.showInformationMessage(
            'Sorting JSON translation files...',
          );
          ...
          if (result.totalFiles === 0) {
            vscode.window.showWarningMessage(
              'No JSON translation files found in common translation directories.',
            );
            return;
          }
          ...
          if (result.sortedFiles === 0) {
            vscode.window.showInformationMessage(
              `All ${result.totalFiles} JSON translation files are already sorted.`,
            );
          } else {
            vscode.window.showInformationMessage(
          ...
          vscode.window.showErrorMessage(
            `Failed to sort JSON translation files: ${error}`,
          );
        }
      },
    ),
