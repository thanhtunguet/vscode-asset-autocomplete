...
export function registerNativeExtractionCommands(
  ...
  context.subscriptions.push(
    ...
    vscode.commands.registerCommand(
      'i18n-autocomplete.nativeExtractAll',
      async () => {
        try {
          vscode.window.showInformationMessage(
            'Starting native extraction for all languages...',
          );
          ...
          vscode.window.showInformationMessage(
            'Native extraction completed for all languages',
          );
        } catch (error) {
    ...
    vscode.commands.registerCommand(
      'i18n-autocomplete.nativeExtract',
      async () => {
        try {
          ...
          vscode.window.showInformationMessage(
            `Starting native extraction for ${selected.config.code}...`,
          );
          ...
          loadTranslationKeys(workspaceFolder.uri.fsPath);
          vscode.window.showInformationMessage(
            `Native extraction completed for ${selected.config.code}`,
          );
        } catch (error) {
          vscode.window.showErrorMessage(`Native extraction failed: ${error}`);
          showLog(`Native extraction error: ${error}`);
    ...
    vscode.commands.registerCommand(
      'i18n-autocomplete.nativeAnalyze',
      async () => {
        try {
          const languageConfigs = getLanguageConfigs();
          ...
          const totalTranslations = results.reduce(
            (sum, result) => sum + result.translations.length,
            0,
          );
          const totalFiles = results.reduce(
            (sum, result) => sum + result.totalFiles,
            0,
          );
          ...
          vscode.window.showInformationMessage(
            `Analysis complete. Found ${totalTranslations} translations in ${totalFiles} files. Check output for details.`,
          );
        } catch (error) {
    ...
    vscode.commands.registerCommand(
      'i18n-autocomplete.configureLanguage',
      async () => {
        try {
          // Get language code
          const code = await vscode.window.showInputBox({
            ...
            placeHolder: 'en',
            validateInput: (text) =>
              text.trim() === '' ? 'Language code cannot be empty' : null,
          });
          ...
          if (languages.includes(code)) {
            vscode.window.showInformationMessage(
              `Language ${code} is already configured`,
            );
            return;
          }
          ...
          vscode.window.showErrorMessage(
            `Failed to configure language: ${error}`,
          );
        }
      },
    ),
    ...
    vscode.commands.registerCommand(
      'i18n-autocomplete.nativeMergeAll',
      async () => {
        try {
          vscode.window.showInformationMessage(
            'Starting translation file merge for all languages...',
          );
          ...
          loadTranslationKeys(workspaceFolder.uri.fsPath);
          vscode.window.showInformationMessage(
            'Translation file merge completed for all languages',
          );
        } catch (error) {
    ...
    vscode.commands.registerCommand(
      ...
      async () => {
        try {
          ...
          vscode.window.showInformationMessage(
            `Starting translation file merge for ${selected.config.code}...`,
          );
          ...
          loadTranslationKeys(workspaceFolder.uri.fsPath);
          vscode.window.showInformationMessage(
            `Translation file merge completed for ${selected.config.code}`,
          );
        } catch (error) {
          vscode.window.showErrorMessage(`Translation merge failed: ${error}`);
          showLog(`Translation merge error: ${error}`);
