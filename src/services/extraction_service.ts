...
import {
  TranslationExtractorFactory,
  type ExtractionResult,
  type ExtractedTranslation,
  BaseTranslationExtractor,
} from './translation_extractor';
...
export class ExtractionService {
  ...
  public async extractAll(): Promise<ExtractionResult[]> {
    ...
    if (languageConfigs.length === 0) {
      showLog(
        'No language configurations found. Please configure projectLanguages in settings.',
      );
      return results;
    }
    ...
    for (const languageConfig of languageConfigs) {
      try {
        ...
        showLog(
          `Error extracting translations for ${languageConfig.code}: ${error}`,
        );
      }
    }
   ...
   */
  public async extractForLanguage(
    ...
    showLog(
      `Found ${files.length} source files to scan for ${languageConfig.code}`,
    );

    // Create appropriate extractor based on configured project language
    const extractor = TranslationExtractorFactory.createExtractor(
      languageConfig,
      languageConfig.projectLanguage,
    );
    ...
    showLog(
      `Extracted ${result.translations.length} unique translations from ${result.totalFiles} files (${result.totalMatches} total matches)`,
    );
   ...
   */
  public async extractAndGenerate(
    languageConfig: LanguageConfig,
  ): Promise<void> {
    const result = await this.extractForLanguage(languageConfig);
   ...
   * Merge translation file for a specific language without resetting existing values
   */
  public async mergeTranslationFile(
    languageConfig: LanguageConfig,
  ): Promise<void> {
   ...
   */
  private async generatePartialFiles(
    ...
    for (const [namespace, keys] of namespaces) {
      ...
      keys.forEach((key) => {
        ...
        updatedTranslations[keyWithoutNamespace] =
          existingTranslations[keyWithoutNamespace] || '';
      });
      ...
      showLog(
        `Updated partial file: ${filePath} (${Object.keys(updatedTranslations).length} keys)`,
      );
    }
  }
  ...
  private async generateMainFile(
    ...
    result.translations.forEach((translation) => {
      ...
      updatedTranslations[translation.key] =
        existingTranslations[translation.key] || '';
    });
    ...
    showLog(
      `Updated main file: ${mainFilePath} (${Object.keys(updatedTranslations).length} keys)`,
    );
  }
   ...
   */
  private async mergePartialFilesToSingleFile(
    languageConfig: LanguageConfig,
  ): Promise<void> {
    ...
    for (const partialFilePath of partialFiles) {
      try {
        ...
        Object.entries(partialTranslations).forEach(([key, value]) => {
          ...
          if (
            existingTranslations[namespacedKey] &&
            existingTranslations[namespacedKey].trim() !== ''
          ) {
        ...
        showLog(
          `Merged partial file: ${partialFilePath} (added namespaced keys with prefix: ${namespace})`,
        );
      } catch (error) {
    ...
    showLog(
      `Merged ${partialFiles.length} partial files into: ${mainFilePath} (${Object.keys(sortedTranslations).length} namespaced keys total)`,
    );
  }
  ...
  private async mergeMainFile(
    ...
    showLog(
      `Merged main file: ${mainFilePath} (${Object.keys(updatedTranslations).length} keys)`,
    );
  }
  ...
  private loadExistingTranslations(filePath: string): TranslationFile {
    try {
      ...
      showLog(
        `Warning: Could not load existing translations from ${filePath}: ${error}`,
      );
    }
  ...
  private getLanguageConfigs(): LanguageConfig[] {
    ...
    const projectLanguage = config.get<'dart' | 'typescript'>(
      'projectLanguage',
      'typescript',
    );
    const translationFunctions = config.get<string[]>('translationFunctions', [
      't',
      'translate',
    ]);
   ...
   */
  private detectProjectTypeFromFiles(
    files: any[],
  ): 'dart' | 'typescript' | 'mixed' {
